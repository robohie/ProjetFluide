...
class ProfilJoukowski(PlanPlus):
    ...
    def _update_potentiel(self):
        ...
        ecoulement_uniforme = self.U_inf * (
        ...
        self.f = (ecoulement_uniforme + tourbillon).subs(
...
def export_plots(objet, a: float, b: float, prefix: str = "ecoulement"):
    ...
    fig_courant = courant(objet, a, b, np.linspace(-2, 2, 15))
    fig_potentiel = potentiel(objet, a, b, np.linspace(-2, 2, 15))
...
if __name__ == "__main__":
    ...
    courant(profil, 3, 3, np.linspace(-2, 2, 15)).suptitle("Profil de Joukowski")
    plt.tight_layout()
    plt.show()