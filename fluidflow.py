...
def plot_niveaux(objet, func, niveaux, a, b, titre: str) -> sympy.plotting.plot.Plot:
    """Trace plusieurs courbes de niveau pour une fonction donnée."""
    figures = []
    for n in niveaux:
        fig = sympy.plot_implicit(func(n), (objet.x, -a, a), (objet.y, -b, b), show=False, adaptive=False, depth=4)
        figures.append(fig)
...
def courant(objet, a, b):
    niveaux = [-3, -2, -1, 1, 2, 3]
    return plot_niveaux(objet, objet.c_niv1, niveaux, a, b, "Fonction de courant de l'écoulement")

def potentiel(objet, a, b):
    niveaux = [-3, -2, -1, 0, 1, 2]
    return plot_niveaux(objet, objet.c_niv2, niveaux, a, b, "Fonction potentielle de l'écoulement")

def champ(objet, x_lim: Tuple[float, float]=(-10, 10), y_lim: Tuple[float, float]=(-10, 10), density: int=20, save_path: Optional[str]=None):
...
if __name__ == "__main__":
    ...
    latex_path = "ProjetMecaniqueFluide.tex"
