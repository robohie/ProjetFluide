...
class _dok_base(_spbase, IndexMixin, dict):
    ...
    def _set_arrayXarray(self, row, col, x):
        ...
        x = x.ravel()
        self._dict._update(zip(zip(row, col), x))
    ...
    def __add__(self, other):
        if isscalarlike(other):
            ...
            with np.errstate(over='ignore'):
                new._dict._update((k, new[k] + v) for k, v in o_items)
        elif isdense(other):
    ...
    def __neg__(self):
        ...
        new._dict._update((k, -v) for k, v in self.items())
        return new
    ...
    def _mul_scalar(self, other):
        ...
        new = self._dok_container(self.shape, dtype=res_dtype)
        new._dict._update(((k, v * other) for k, v in self.items()))
        return new
    ...
    def __imul__(self, other):
        if isscalarlike(other):
            self._dict._update((k, v * other) for k, v in self.items())
            return self
        return NotImplemented
    ...
    def transpose(self, axes=None, copy=False):
        ...
        new._dict._update((((right, left), val) for (left, right), val in self.items()))
        return new
