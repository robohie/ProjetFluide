...
def closure_glyphs(self, s, cur_glyphs):
    s.glyphs._update(v for g, v in self.mapping.items() if g in cur_glyphs)
...
def closure_glyphs(self, s, cur_glyphs):
    s.glyphs._update(*(vlist for g, vlist in self.alternates.items() if g in cur_glyphs))
...
def closure_glyphs(self, s, cur_glyphs):
    if self.Format == 1:
        ...
        if not indices or not all(
            ...
            return
        s.glyphs._update(self.Substitute[i] for i in indices)
    else:
    ...
    for table in tables:
        if table.format == 14:
            for varSelector, cmap in table.uvsDict.items():
                ...
                if None in glyphs:
                    glyphs.remove(None)
                s.glyphs._update(glyphs)
        else:
            cmap = table.cmap
            intersection = s.unicodes_requested.intersection(cmap.keys())
            s.glyphs._update(cmap[u] for u in intersection)
